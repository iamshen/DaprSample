...
///     EnumTypeExtensions
/// </summary>
public static class SwaggerEnumTypeExtensions
{
    #region Methods
    ...
    internal static List<OpenApiString> GetEnumValuesDescription(Type enumType, DescriptionSources descriptionSource,
        IEnumerable<XPathNavigator> xmlNavigators, bool includeRemarks = false)
    {
    ...
    private static string GetNodeNameForMember(MemberInfo memberInfo)
    {
        ...
        stringBuilder.Append(QualifiedNameFor(memberInfo.DeclaringType));
    ...
    private static string QualifiedNameFor(Type type, bool expandGenericArgs = false)
    {
        ...
        if (type.IsConstructedGenericType & expandGenericArgs)
        {
            ...
            var values = type.GetGenericArguments().Select(t =>
                !t.IsGenericParameter ? QualifiedNameFor(t, true) : string.Format("`{0}", t.GenericParameterPosition));
    ...
    internal static string AddEnumValuesDescription(this OpenApiSchema schema, string xEnumNamesAlias,
        string xEnumDescriptionsAlias, bool includeDescriptionFromAttribute = false)
    {
        ...
        for (var i = 0; i < schema.Enum.Count; i++)
        {
            ...
            if (includeDescriptionFromAttribute)
            {
                ...
                {
                    var description = ((OpenApiString)((OpenApiArray)schema.Extensions[xEnumDescriptionsAlias])[i])
                        .Value;
