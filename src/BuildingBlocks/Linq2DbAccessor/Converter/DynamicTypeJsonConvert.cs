...
public class DynamicTypeJsonConvert<T> : JsonConverter<T>
{
    ...
    private Type? GetTypeFromString(string? moduleName)
    {
        ...
        if (_typeCaches.TryGetValue(moduleName, out var s)) return s;

        var newType = Type.GetType(moduleName);
        if (newType is not null)
            _typeCaches.AddOrUpdate(moduleName, newType, (_, _) => newType);
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private T? ReadNumber(ref Utf8JsonReader reader, Type type)
    {
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private T? ReadString(ref Utf8JsonReader reader, Type type)
    {
    ...
    private void WriteObject(ref Utf8JsonWriter writer, object? value, JsonSerializerOptions options)
    {
        ...
        else
        {
            ...
            writer.WriteStringValue(valueType.IsAnonymousType()
                ? $"{valueType.FullName}"
                : $"{valueType.FullName}, {valueType.Assembly.GetName().Name}");
    ...
    public override T? Read(ref Utf8JsonReader reader, Type type, JsonSerializerOptions options)
    {
        if (reader.TokenType == JsonTokenType.String)
            return ReadString(ref reader, type);

        if (reader.TokenType == JsonTokenType.Number)
            return ReadNumber(ref reader, type);
    ...
    ///     类型缓存
    /// </summary>
    private readonly ConcurrentDictionary<string, Type> _typeCaches = new();
    ...
    private object? ReadObject(JsonElement jsonElement, Type? type, JsonSerializerOptions options)
    {
            ...
            while (jsonProperties.MoveNext())
                if (jsonProperties.Current.Name != AppendTypeName)
                {
                    dynamicObject.TryAdd(jsonProperties.Current.Name,
                        ReadValue(jsonProperties.Current.Value, options));
                }
                else
                {
        ...
        else
        {
            ...
            instance = jsonElement.TryGetProperty(AppendValueName, out var valueProperty)
                ? valueProperty.Deserialize(moduleType, options)
                : jsonElement.Deserialize(moduleType, options);
        }
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private object? ReadValue(JsonElement jsonElement, JsonSerializerOptions options)
    {
        ...
        if (jsonElement.ValueKind == JsonValueKind.Array)
        {
            ...
            foreach (var item in jsonElement.EnumerateArray())
                list.Add(ReadValue(item, options));
