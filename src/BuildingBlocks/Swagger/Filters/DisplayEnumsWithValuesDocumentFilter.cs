...
internal class DisplayEnumsWithValuesDocumentFilter : IDocumentFilter
{
    ...
    public DisplayEnumsWithValuesDocumentFilter(IOptions<FixEnumsOptions> options,
    ...
    public void Apply(OpenApiDocument openApiDoc, DocumentFilterContext context)
    {
        ...
        foreach (var schemaDictionaryItem in openApiDoc.Components.Schemas)
        {
            ...
            var description = schema.AddEnumValuesDescription(_xEnumNamesAlias, _xEnumDescriptionsAlias,
                _includeDescriptionFromAttribute);
            if (description != null)
            {
                if (schema.Description == null)
                    schema.Description = description;
                else if (!schema.Description.Contains(description)) schema.Description += description;
            }
        }
        ...
        foreach (var parameter in openApiDoc.Paths.Values.SelectMany(v => v.Operations)
                     .SelectMany(op => op.Value.Parameters))
        {
            ...
            if (parameter.Schema?.Reference == null)
            {
                if (parameter.Schema?.AllOf?.Count > 0)
                    schema = context.SchemaRepository.Schemas.FirstOrDefault(s =>
                        parameter.Schema.AllOf.FirstOrDefault(a => a.Reference.Id == s.Key) != null).Value;
                else
                    continue;
            }
            else
            ...
            if (schema != null)
            {
                ...
                if (description != null)
                {
                    ...
                    else if (!parameter.Description.Contains(description)) parameter.Description += description;
                }
            }
        }
