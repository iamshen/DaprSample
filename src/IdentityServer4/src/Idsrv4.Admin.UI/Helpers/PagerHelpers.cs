...
{
    public static int GetTotalPages(int pageSize, int totalCount) => (int)Math.Ceiling((double)totalCount / pageSize);

    public static bool IsActivePage(int currentPage, int currentIteration) => currentPage == currentIteration;
    ...
    public static string GetUrl(string baseUrl, QueryString queryString) => $"{baseUrl}{queryString}";
}